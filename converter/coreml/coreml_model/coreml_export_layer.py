...
import torch
from helpers.constants import XY_SLICE, WH_SLICE, SCORE_SLICE, CLASSES_SLICE, BLUE, END_COLOR, DEFAULT_CONF_THRESHOLD, \
    DEFAULT_IOU_THRESHOLD, IOU_NAME, CONF_NAME, DEFAULT_MAX_NUMBER_DETECTION, MASKS_NAME, \
    CONFIDENCE_NAME, COORDINATES_NAME, NUMBER_NAME, ULTRALYTICS, YOLOv5


class CoreMLSegmentationExportLayerGenerator:
    ...
    def add_to(self, builder):
        """ Creates segmentation export logic and adds it to the builder
        ...
        masks_resolution = self.pt_model.model_parameters.input_resolution // 4

        all_predictions = "all_predictions" if self.pt_model.model_parameters.model_orig == YOLOv5 else "all_predictions_transposed"

        if self.pt_model.model_parameters.model_orig == ULTRALYTICS:
            builder.add_transpose(name=f"transpose_predictions", axes=[0, 2, 1], input_name="all_predictions",
                                  output_name="all_predictions_transposed")
        ...
        builder.add_scale(name=f"normalize_coordinates", input_name=f"sliced_coordinates",
                          ...
                          W=torch.tensor([1 / self.pt_model.model_parameters.input_resolution]).numpy(), b=0,
                          has_bias=False)

        ### Confidence calculation ###
        if self.pt_model.model_parameters.model_orig == YOLOv5:
            ...
            builder.add_slice(name=f"slice_label_confidence", input_name=all_predictions,
                              ...
                              end_index=CLASSES_SLICE[0] + self.pt_model.number_of_classes)
            ...
            builder.add_slice(name=f"slice_classes", input_name=all_predictions,
                              output_name=f"temp_raw_confidence", axis="width", start_index=CLASSES_SLICE[0] - 1,
                              end_index=CLASSES_SLICE[0] - 1 + self.pt_model.number_of_classes)
        ...
        if self.pt_model.model_parameters.model_orig == YOLOv5:
            builder.add_slice(name=f"slice_masks", input_name=all_predictions,
                              ...
                              start_index=CLASSES_SLICE[0] + self.pt_model.number_of_classes,
                              end_index=0)
        else:
            builder.add_slice(name=f"slice_masks", input_name=all_predictions,
                              output_name=f"sliced_masks", axis="width",
                              start_index=CLASSES_SLICE[0] - 1 + self.pt_model.number_of_classes,
                              end_index=0)
        ...
        builder.add_resize_bilinear(name="resize_masks", input_name="final_masks", output_name="resized_masks",
                                    target_height=self.pt_model.model_parameters.input_resolution,
                                    target_width=self.pt_model.model_parameters.input_resolution,
                                    mode="UPSAMPLE_MODE")
        ...
        builder.set_output(output_names=[COORDINATES_NAME, CONFIDENCE_NAME, MASKS_NAME, NUMBER_NAME], output_dims=[
            (DEFAULT_MAX_NUMBER_DETECTION, self.pt_model.number_of_classes), (DEFAULT_MAX_NUMBER_DETECTION, 4),
