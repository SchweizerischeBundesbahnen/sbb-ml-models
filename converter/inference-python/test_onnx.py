...
def test_onnx(reference_model, model_dir, img_dir, verbose):
    logging.basicConfig(format='%(asctime)s %(message)s', datefmt="%Y-%m-%d %H:%M:%S", level=logging.INFO)
    logging.info("\n----------------------------------------------------------------------")
    logging.info("ONNX Test")

    model_dir_path = Path(model_dir)

    if not model_dir_path.exists():
        print(f"{RED}Directory not found{END_COLOR}: '{model_dir}'")
        exit(1)

    models = [str(m) for m in model_dir_path.glob('*.onnx')]
...
if __name__ == '__main__':
    ...
    test_onnx(opt.reference_model, opt.model_dir, opt.img_dir, opt.verbose)
