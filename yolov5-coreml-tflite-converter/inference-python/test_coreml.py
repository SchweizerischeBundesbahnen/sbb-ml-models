...
from helpers.constants import END_COLOR, RED, DEFAULT_MODEL_OUTPUT_DIR
from test_model import ModelTest


def test_coreml(reference_model, model_dir, img_dir, verbose):
    logging.basicConfig(format='%(asctime)s %(message)s', datefmt="%Y-%m-%d %H:%M:%S", level=logging.INFO)
    logging.info("\n----------------------------------------------------------------------")
    logging.info("CoreML Test")
    ...
    models = [str(m) for m in model_dir_path.glob('*.mlmodel')]
...
if __name__ == '__main__':
    ...
    test_coreml(opt.reference_model, opt.model_dir, opt.img_dir, opt.verbose)
